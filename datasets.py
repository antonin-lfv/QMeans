...
def generate_dataset(k=3, n_data_points_per_class=50):
    ...
    x = np.random.normal(
        loc=true_centroids_x[0],
        scale=true_centroids_var_x[0],
        size=n_data_points_per_class,
    )
    y = np.random.normal(
        loc=true_centroids_y[0],
        scale=true_centroids_var_y[0],
        size=n_data_points_per_class,
    )
    ...
    for i in range(1, k):
        ...
        x = np.vstack(
            (
                x,
                np.random.normal(
                    loc=true_centroids_x[i],
                    scale=true_centroids_var_x[i],
                    size=n_data_points_per_class,
                ),
            )
        )
        y = np.vstack(
            (
                y,
                np.random.normal(
                    loc=true_centroids_y[i],
                    scale=true_centroids_var_y[i],
                    size=n_data_points_per_class,
                ),
            )
        )
...
def load_dataset(name="toy"):
    ...
    ##############################
        ...
        data = skdata.make_blobs(
            n_samples=200, n_features=2, centers=3, cluster_std=1.8, random_state=101
        )
    ...
    ##############################
        ...
        df = pd.read_csv("kmeans_data.csv", usecols=["Feature 1", "Feature 2", "Class"])
